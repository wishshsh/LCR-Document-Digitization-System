...
The old check_cer.py just read c['val_cer'] from the checkpoint —
a number saved at training time on clean synthetic images.
It never ran the model at all. That's why it always showed 4.13%.
...
# ─────────────────────────────────────────────────────────────────────────────
# NORMALIZER (same fixed pipeline as inference.py)
# ─────────────────────────────────────────────────────────────────────────────

class AdaptiveImageNormalizer:
    """
    Fixed pipeline order to match training data exactly:
      crop -> resize (grayscale) -> binarize LAST
    Produces pure 0/255 binary images like the training set.
    """
    def __init__(self, target_height=64, target_width=400):
    ...
    def normalize(self, img):
        ...
        gray   = self._smart_resize_gray(gray)      # resize on grayscale
        binary = self._binarize(gray)               # binarize AFTER resize
        return binary
...
# ─────────────────────────────────────────────────────────────────────────────
# LIVE CER MEASUREMENT
# ─────────────────────────────────────────────────────────────────────────────

def measure_live_cer(model, idx_to_char, img_h, img_w,
                     ann_file, data_dir, device, max_samples=200):
    """
    Actually run the model on images and compute real CER/WER/accuracy.
    This is what check_cer should have always done.
    """
    ...
    with torch.no_grad():
        for ann in annotations:
            ...
            try:
                ...
                if cd > 0: worst_errors.append((gt, pred, cd))
                n_evaluated += 1
            except Exception:
                continue
...
# ─────────────────────────────────────────────────────────────────────────────
# FOLDER MODE (no ground truth needed)
# ─────────────────────────────────────────────────────────────────────────────
...
if USE_SAVED:
    # ── Old mode: read saved CER from checkpoint (unreliable) ─────────────
    print("=" * 65)
    ...
    for cp in CHECKPOINTS:
        ...
        try:
            ...
            if cer < best_cer: best_cer, best_cp = cer, cp
        except Exception as e:
    ...
    # ── Live mode: actually run the model ─────────────────────────────────
    print("=" * 78)
    ...
    for cp in CHECKPOINTS:
        ...
        try:
            ...
            img_w       = config.get('img_width', 400)
            ...
            # Warn if live CER differs greatly from saved CER
